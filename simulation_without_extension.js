  ...
  StartglobalAggregation() {
    ...
    let serverloop = setInterval(() => {
      if (this.aggregation_queue.length > 0) {
        ...
        this.tb += 1;
        console.log("Extent of updates", this.extent_of_updates);
        ...
        if (this.tb == this.recluster_threshold) {
          ...
          for (i = 0; i < edge_nodes.length; i++) {
            ...
            edge_nodes[
              leaderNodes[edge_nodes[i].clusterid]
            ].model_queue.length = 0;
          }
          setTimeout(() => {
            this.Reclustering();
          }, 20);
    ...
    let train = setInterval(() => {
      ...
      if (
        ...
        if (this.training == false) {
          console.log(
            `Total Time wasted to train node ${this.node_num}`,
